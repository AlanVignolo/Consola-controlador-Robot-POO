...
float Interpolation::setInterpolation(float px, float py, float pz, float pe, float v) { // cambiado de void a float para retornar tiempo
  Point p;
  p.xmm = px;
  p.ymm = py;
  p.zmm = pz;
  p.emm = pe;
  return setInterpolation(p, v); //agregado retorno de tiempo
}
...
float Interpolation::setInterpolation(Point p1, float v) { // cambiado de void a float para retornar tiempo
  Point p0;
  p0.xmm = xStartmm + xDelta;
  p0.ymm = yStartmm + yDelta;
  p0.zmm = zStartmm + zDelta;
  p0.emm = eStartmm + eDelta;
  return setInterpolation(p0, p1, v); //agregado retorno de tiempo
}

float Interpolation::setInterpolation(Point p0, Point p1, float av) { // cambiado de void a float para retornar tiempo
  v = av; //mm/s
  ...
  // en caso de que la distancia a recorrer sea nula el tiempo de operacion es cero
  if (dist == 0.0) {
    return 0.0;
  }
  else {
    return tmul; //agregado retorno de tiempo
  }
}
...
bool Interpolation::isAllowedPosition(float pos_tracker[4]) {
  ...
  if (!retVal) {
    //Logger::logERROR("LIMIT REACHED: [X:" + String(pos_tracker[X_AXIS]) + " Y:" + String(pos_tracker[Y_AXIS]) + " Z:" + String(pos_tracker[Z_AXIS]) + " E:" + String(pos_tracker[E_AXIS]) + "]"); //respuesta original
    Logger::logERROR("POINT IS OUTSIDE OF WORKSPACE");  //respuesta en modo de operacion modificado
  }
  return retVal;
}
